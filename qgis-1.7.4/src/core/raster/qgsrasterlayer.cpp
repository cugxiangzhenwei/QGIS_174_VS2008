...
{
  QString strError = mDataProvider->buildPyramids( theRasterPyramidList, theResamplingMethod, theTryInternalFlag );
  if (strError == "")
	  mHasPyramids = true;
...
bool QgsRasterLayer::draw( QgsRenderContext& rendererContext )
{
  ...
  /***************************卷帘处理********************************/
  int pixelWidth =  theViewExtent.width()  / theQgsMapToPixel.mapUnitsPerPixel() + 0.5;	
  int pixelHeight = theViewExtent.height() / theQgsMapToPixel.mapUnitsPerPixel() + 0.5;	

  // Can we reuse the previously cached image?
  if ( cachedImage &&
	  cachedViewExtent == theViewExtent &&
	  ...
	  cachedViewHeight == pixelHeight )
  {
	  ...
	  {
		  //是否需要调整亮度透明度
		  if ((mBrightnessLevel != 0)||(mContrastLevel != 100)||(mGammaLevel != 1)||(mTransparencyLevel < 255))
		  {
			  if(cachedImage2Swip == NULL)
			  {
				  QImage tempImage = enhancedImage(*cachedImage);

				  cachedImage2Swip = new QImage( pixelWidth, pixelHeight, QImage::Format_ARGB32 );
				  cachedImage2Swip->fill( 0 );

				  QPainter p( cachedImage2Swip );
				  p.drawImage( 0,0, tempImage );	
			  }

			  theQPainter->drawImage( mShowSwipeRect.left(),mShowSwipeRect.top(), *cachedImage2Swip,
				  mShowSwipeRect.left(),mShowSwipeRect.top(),mShowSwipeRect.width(),mShowSwipeRect.height() );
		  }
		  else
		  {
			  theQPainter->drawImage( mShowSwipeRect.left(), mShowSwipeRect.top(), *cachedImage,
				  mShowSwipeRect.left(),mShowSwipeRect.top(),mShowSwipeRect.width(),mShowSwipeRect.height() );
		  }
	  ...
	  else
	  {
		  ...
		  {
			  QImage tempImage = enhancedImage(*cachedImage);
			  theQPainter->drawImage( 0,0, tempImage );	
		  }
		  else
			  theQPainter->drawImage( 0,0, *cachedImage );		
	  }
  }
  ...
  draw( &p, myRasterViewPort, &theQgsMapToPixel );
  ...
  switch ( mDrawingStyle )
  {
    ...
    case PalettedSingleBandGray:
      ...
      else
      {
        ...
        int myBandNo = 1;
        drawPalettedSingleBandGray( theQPainter, theRasterViewPort, theQgsMapToPixel, myBandNo );
    ...
    case PalettedSingleBandPseudoColor:
      ...
      {
        int myBandNo = 1;
        drawPalettedSingleBandPseudoColor( theQPainter, theRasterViewPort, theQgsMapToPixel, myBandNo );
        break;
      }
    ...
    case MultiBandSingleBandGray:
      ...
      else
      {
        ...
        drawMultiBandSingleBandGray( theQPainter, theRasterViewPort, theQgsMapToPixel, bandNumber( mGrayBandName ) );
        break;
      }
    ...
    case MultiBandSingleBandPseudoColor:
      ...
      {
        drawMultiBandSingleBandPseudoColor( theQPainter, theRasterViewPort, theQgsMapToPixel, bandNumber( mGrayBandName ) );
        break;
      }
    ...
    case MultiBandColor:
      ...
      {
        drawMultiBandColor( theQPainter, theRasterViewPort, theQgsMapToPixel );
      }
      break;
	case MultiBandCustomColor:	  //用户自定义的颜色显示方案
		{
			drawMultiBandCustomColor( theQPainter, theRasterViewPort, theQgsMapToPixel );
		}
		break;
	case MultiBandLUT:	  //使用多波段LUT查找表拉伸显示
		{
			drawMultiBandLut( theQPainter, theRasterViewPort, theQgsMapToPixel );
		}
		break;
	case MultiBandHSL:	  //色相饱和度显示
		{
			drawMultiBandHSL( theQPainter, theRasterViewPort, theQgsMapToPixel );
		}
		break;
    case SingleBandColorDataStyle:
      ...
      {
        drawSingleBandColorData( theQPainter, theRasterViewPort, theQgsMapToPixel, bandNumber( mGrayBandName ) );
        break;
      }
...
QgsRasterBandStats QgsRasterLayer::GetBandStats( int theBandNo, int theBinCount, bool theIgnoreOutOfRangeFlag, bool theHistogramEstimatedFlag )
{
	QgsRasterBandStats myRasterBandStats = bandStatistics( theBandNo );
	mDataProvider->populateHistogram( theBandNo, myRasterBandStats, theBinCount, theIgnoreOutOfRangeFlag, theHistogramEstimatedFlag );
	return myRasterBandStats;
...
void QgsRasterLayer::init()
{
  ...
  for(int i=0; i<256; i++)//输出的对照表，默认和原始一样
  {
	  m_LUTA[i] = i;
	  m_LUTR[i] = i;
	  m_LUTG[i] = i;
  ...
  memset(m_iHue, 0, sizeof(int)*7);
  memset(m_iSaturation, 0, sizeof(int)*7);
  memset(m_iLightness, 0, sizeof(int)*7);
}
                                      ...
                                      bool loadDefaultStyleFlag )
{
  ...
  mHasPyramids = mDataProvider->hasPyramids();
...
void QgsRasterLayer::setContrastEnhancementAlgorithm( QString theAlgorithm, bool theGenerateLookupTableFlag )
{
  ...
  else if ( theAlgorithm == "Clip2PrecentToMinMax"  )
  {
	  setContrastEnhancementAlgorithm( QgsContrastEnhancement::Clip2PercentToMinMax, theGenerateLookupTableFlag );
  }
...
void QgsRasterLayer::GetHistoMinMax(QgsRasterBandStats * bandStats, double &dMin,double &dMax )
{
	QgsRasterBandStats::HistogramVector * histogramVector = bandStats->histogramVector;
	double iMinValue = bandStats->minimumValue;
	double iMaxValue = bandStats->maximumValue;

	int iSize = histogramVector->size();//直方图个数
	if (iSize == 0)
	{
		dMax = 255;
		dMin = 0;
	...
	double dSum = 0.0;
	int iCountZero = (*histogramVector)[0];
	for (int i=0; i<iSize; i++)
		dSum += (*histogramVector)[i];
	
	double icount = 0.001 * ( dSum - iCountZero);//有效的象元个数
	...
	for (int i=1; i<iSize; i++)
	{
		...
		if(dSum > icount)
		{
			dMin = (iMaxValue-iMinValue) / 255.0 * i + iMinValue;;
			break;
		}
	}
	...
	for(int i=255; i>0; i--)
	{
		...
		if(dSum > icount)
		{
			dMax = (iMaxValue-iMinValue) / 255.0 * i + iMinValue;;
			break;
		}
	}
}
    ...
    const QgsMapToPixel* theQgsMapToPixel )
{
  ...
  /*
   * If a stretch is requested and there are no user defined Min Max values
   * we need to get these values from the bands themselves.
   *
   */
  QgsContrastEnhancement::ContrastEnhancementAlgorithm enh = contrastEnhancementAlgorithm();
  if ( QgsContrastEnhancement::NoEnhancement != enh && !mUserDefinedRGBMinimumMaximum && mStandardDeviations > 0 )
  {
  ...
  else if ( QgsContrastEnhancement::NoEnhancement != enh && !mUserDefinedRGBMinimumMaximum )
  {
	  //2% linear stretch
	  if (QgsContrastEnhancement::Clip2PercentToMinMax == enh)
	  {
		  populateHistogram(myRedBandNo, 256, false, true);
		  populateHistogram(myGreenBandNo, 256, false, true);
		  populateHistogram(myBlueBandNo, 256, false, true);
		  ...
		  //这里计算用于拉伸的两个值min,max,需要用直方图，计算累计直方图，然后去掉两头2%的部分，剩下的范围，比这个（0，255）小,进行线性拉伸
		  double dRedMin = 0.0;
		  double dRedMax = 0.0;
		  ...
		  //计算累计直方图,find min max
		  GetHistoMinMax(&myRedBandStats,dRedMin,dRedMax);
		  GetHistoMinMax(&myGreenBandStats,dGreenMin,dGreenMax);
		  GetHistoMinMax(&myBlueBandStats,dBlueMin,dBlueMax);

		  setMaximumValue( myRedBandNo, dRedMax );
		  setMinimumValue( myRedBandNo, dRedMin );
		  setMaximumValue( myGreenBandNo, dGreenMax );
		  setMinimumValue( myGreenBandNo, dGreenMin );
		  setMaximumValue( myBlueBandNo, dBlueMax);
		  setMinimumValue( myBlueBandNo, dBlueMin);
	  }
	  else
	  {
    ...
    const QgsMapToPixel* theQgsMapToPixel, int theBandNo )
{
  ...
  {
	  //2% linear stretch
	  if (QgsContrastEnhancement::Clip2PercentToMinMax == contrastEnhancementAlgorithm())
	  {
		  populateHistogram(theBandNo, 256, false, true);
		  myGrayBandStats = bandStatistics( theBandNo );
		  ...
		  //计算累计直方图,find min max
		  GetHistoMinMax(&myGrayBandStats,dGrayMin,dGrayMax);

		  setMaximumValue( theBandNo, dGrayMax );
		  setMinimumValue( theBandNo, dGrayMin );
	  }
	  else
	  {
		  //This case will be true the first time the image is loaded, so just approimate the min max to keep
...
inline float Cover2RGB(double *pData, int iSize, float* pSrcData)
{
	double dResult = 0.0;
	for (int i=0; i<iSize; i++)
		dResult += (pData[i]*pSrcData[i]);
	
	return static_cast<float> (dResult+0.5);
}

// 显示用户自定义颜色
void QgsRasterLayer::drawMultiBandCustomColor( QPainter * theQPainter, QgsRasterViewPort * theRasterViewPort,
											  const QgsMapToPixel* theQgsMapToPixel )
{
	//如果没有设置自定义参数,调用以前方法
	if (mCustomBands == NULL)
	...
	if (mBandCount == 4)
	{
		myForthType = mDataProvider->dataType( myForthBandNo );	
	}
	else if (mBandCount >= 5)
	{
		myForthType = mDataProvider->dataType( myForthBandNo );	
		myFiveType = mDataProvider->dataType( myFiveBandNo );	
	}

	QRgb* redImageScanLine = NULL;
	void* redRasterScanLine = NULL;

	QRgb* greenImageScanLine = NULL;	
	void* greenRasterScanLine = NULL;

	QRgb* blueImageScanLine = NULL;
	void* blueRasterScanLine = NULL;

	QRgb* forthImageScanLine = NULL;
	void* forthRasterScanLine = NULL;
	QRgb* fiveImageScanLine = NULL;
	void* fiveRasterScanLine = NULL;
	...
	double myGreenValue = 0.0;
	double myBlueValue = 0.0;
	double myForthValue = 0.0;
	double myFiveValue = 0.0;

	int myStretchedRedValue   = 0;
	int myStretchedGreenValue = 0;
	int myStretchedBlueValue  = 0;

	int myCustomRValue = 0;
	int myCustomGValue = 0;
	int myCustomBValue = 0;

	int myAlphaValue = 0;

	float fSrcData[5] = {0.0};

	QgsContrastEnhancement* myRedContrastEnhancement = contrastEnhancement( myRedBandNo );
	QgsContrastEnhancement* myGreenContrastEnhancement = contrastEnhancement( myGreenBandNo );
	QgsContrastEnhancement* myBlueContrastEnhancement = contrastEnhancement( myBlueBandNo );

	QgsRasterImageBuffer *pRedImageBuffer = NULL;
	QgsRasterImageBuffer *pGrnImageBuffer = NULL;
	QgsRasterImageBuffer *pBluImageBuffer = NULL;
	QgsRasterImageBuffer *pFthImageBuffer = NULL;
	QgsRasterImageBuffer *pFveImageBuffer = NULL;

	pRedImageBuffer = new QgsRasterImageBuffer( mDataProvider, myRedBandNo, theQPainter, theRasterViewPort, theQgsMapToPixel, &mGeoTransform[0] );
	pRedImageBuffer->reset();

	pGrnImageBuffer = new QgsRasterImageBuffer( mDataProvider, myGreenBandNo, theQPainter, theRasterViewPort, theQgsMapToPixel, &mGeoTransform[0] );
	pGrnImageBuffer->setWritingEnabled( false ); //only draw to redImageBuffer
	pGrnImageBuffer->reset();

	pBluImageBuffer = new QgsRasterImageBuffer( mDataProvider, myBlueBandNo, theQPainter, theRasterViewPort, theQgsMapToPixel, &mGeoTransform[0] );
	pBluImageBuffer->setWritingEnabled( false ); //only draw to redImageBuffer
	pBluImageBuffer->reset();

	if (mBandCount == 4)
	{
		pFthImageBuffer = new QgsRasterImageBuffer( mDataProvider, myForthBandNo, theQPainter, theRasterViewPort, theQgsMapToPixel, &mGeoTransform[0] );
		pFthImageBuffer->setWritingEnabled( false ); //only draw to redImageBuffer
		pFthImageBuffer->reset();
	}
	else if (mBandCount >= 5)
	{
		pFthImageBuffer = new QgsRasterImageBuffer( mDataProvider, myForthBandNo, theQPainter, theRasterViewPort, theQgsMapToPixel, &mGeoTransform[0] );
		...
		pFveImageBuffer = new QgsRasterImageBuffer( mDataProvider, myFiveBandNo, theQPainter, theRasterViewPort, theQgsMapToPixel, &mGeoTransform[0] );
		pFveImageBuffer->setWritingEnabled( false ); //only draw to redImageBuffer
		pFveImageBuffer->reset();
	}

	while ( pRedImageBuffer->nextScanLine( &redImageScanLine, &redRasterScanLine )
		&& pGrnImageBuffer->nextScanLine( &greenImageScanLine, &greenRasterScanLine )
		&& pBluImageBuffer->nextScanLine( &blueImageScanLine, &blueRasterScanLine ) )
	{
		if (myForthType != 0)	
			pFthImageBuffer->nextScanLine(&forthImageScanLine,&forthRasterScanLine);
		if (myFiveType != 0)	
			pFveImageBuffer->nextScanLine(&fiveImageScanLine,&fiveRasterScanLine);
		...
		for ( int i = 0; i < theRasterViewPort->drawableAreaXDim; ++i )
		{
			...
			if (myForthType != 0)	
				myForthValue = readValue( forthRasterScanLine, myForthType, i );
			if (myFiveType != 0)	
				myFiveValue = readValue( fiveRasterScanLine, myFiveType, i );
			...
			fSrcData[0] = myStretchedRedValue;
			fSrcData[1] = myStretchedGreenValue;
			fSrcData[2] = myStretchedBlueValue;
			fSrcData[3] = myForthValue;
			fSrcData[4] = myFiveValue;

			float newR = Cover2RGB(mCustomBands   , 5, fSrcData); 
			float newG = Cover2RGB(mCustomBands+ 5, 5, fSrcData);
			float newB = Cover2RGB(mCustomBands+10, 5, fSrcData);

			//对于超过255的值取255
			myCustomRValue = static_cast<unsigned char> (newR>255 ? 255 : newR);
			myCustomGValue = static_cast<unsigned char> (newG>255 ? 255 : newG);
			myCustomBValue = static_cast<unsigned char> (newB>255 ? 255 : newB);		

			redImageScanLine[ i ] = qRgba( myCustomRValue, myCustomGValue, myCustomBValue, myAlphaValue );
		}
	}

	if (pRedImageBuffer != NULL)
		delete pRedImageBuffer;

	if (pGrnImageBuffer != NULL)
		delete pGrnImageBuffer;

	if (pBluImageBuffer != NULL)
		delete pBluImageBuffer;

	if (pFthImageBuffer != NULL)
		delete pFthImageBuffer;

	if (pFveImageBuffer != NULL)
		delete pFveImageBuffer;
}

void QgsRasterLayer::drawMultiBandLut( QPainter * theQPainter, QgsRasterViewPort * theRasterViewPort, const QgsMapToPixel* theQgsMapToPixel)
{
	if (m_bISRGB)
	{
		int myRedBandNo = bandNumber( mRedBandName );
		...
		*/
			...
			&& ( !transparencyImageBuffer || transparencyImageBuffer->nextScanLine( &transparencyImageScanLine, &transparencyRasterScanLine ) ) )
		{
			for ( int i = 0; i < theRasterViewPort->drawableAreaXDim; ++i )
			{
				...
				myStretchedRedValue   = m_LUTR[myStretchedRedValue];
				myStretchedGreenValue = m_LUTG[myStretchedGreenValue];
				myStretchedBlueValue  = m_LUTB[myStretchedBlueValue];
	...
	{
		int theBandNo = bandNumber( mGrayBandName );
		if ( 0 >= theBandNo )
			return;
...
//设置转换参数，传入一个double指针，默认是一个double[3][4]
void QgsRasterLayer::setCustomBands(double const *cusbands, int iSize)
{
	if (mCustomBands != NULL) 
	{
		delete mCustomBands;
	...
	mISize = iSize;
	mCustomBands = new double[iSize];
	memcpy(mCustomBands,cusbands,sizeof(double)*iSize);

	mRedraw = true;
}

//得到当前图层自定义的颜色方案
void QgsRasterLayer::getCustomBands(double *cusbands, int &iSize)
{
	if (mCustomBands == NULL) 
		return;

	iSize = mISize;
	memcpy(cusbands, mCustomBands,sizeof(double)*iSize);
}

//调整亮度对比度透明度
QImage QgsRasterLayer::enhancedImage( const QImage& image)
{
	//亮度对照表
	int brightnessTable[ 256 ];//亮度对照表
	int contrastTable[256];//对比度对照表
	int gammaTable[256];//gamma对照表

	for( int i = 0;i < 256;++i )
	{
		brightnessTable[ i ] = changeBrightness( i, mBrightnessLevel );
		contrastTable[ i ] = changeContrast( i, mContrastLevel );
		gammaTable[ i ] = i;
		//gammaTable[ i ] = changeGamma( i, mGammaLevel );
	}

	QImage transparentImageCopy(image);
	transparentImageCopy.detach();
	...
	for ( int myHeightRunner = 0; myHeightRunner < myHeight; myHeightRunner++ )
	{
		...
		for ( int myWidthRunner = 0; myWidthRunner < myWidth; myWidthRunner++ )
		{
			...
			//亮度、对比度、gamma变换
			int iR = gammaTable[contrastTable[brightnessTable[qRed( myRgb )]]];
			int iG = gammaTable[contrastTable[brightnessTable[qGreen( myRgb )]]];
			int iB = gammaTable[contrastTable[brightnessTable[qBlue( myRgb )]]];

			myLineBuffer[ myWidthRunner ] = qRgba( iR, iG, iB, newTransparency );
		}
	}

	return transparentImageCopy; 
}

//得到用户调色后对照表,为int型256数组
void QgsRasterLayer::getLookupTable(int theBand,int *lutable)
{
	if ( 0 == lutable ) 
	{ return; }
	...
	{
		//得到拉伸对照表
		QgsContrastEnhancement* myContrastEnhancement = contrastEnhancement( theBand );
		...
		for( int i = 0;i < 256;++i )
		{
			...
			contrastTable[ i ] = changeContrast( i, mContrastLevel );
			gammaTable[ i ] = i;
			histoTable[ i ] = myContrastEnhancement->enhanceContrast(i);
			lutable[i] = i;
			//gammaTable[ i ] = changeGamma( i, mGammaLevel );
		}
		...
		for( int i = 0;i < 256;++i )
		{
			lutable[i] = contrastTable[brightnessTable[histoTable[i]]];
		}
	}
}

/** \brief 设置拉伸表，false为灰度图像，true为所有波段都使用该拉伸表 */
void QgsRasterLayer::setLookupTable(byte *pLutG, bool bIsRGB)
{
	m_bISRGB = bIsRGB;
	if (m_bISRGB)
	{
		memset(m_LUTA, 0, sizeof(byte)*256);
		memcpy(m_LUTR, pLutG, sizeof(byte)*256);
		memcpy(m_LUTG, pLutG, sizeof(byte)*256);
		memcpy(m_LUTB, pLutG, sizeof(byte)*256);
	}
	else
	{
		memcpy(m_LUTA, pLutG, sizeof(byte)*256);
		memset(m_LUTR, 0, sizeof(byte)*256);
		memset(m_LUTG, 0, sizeof(byte)*256);
		memset(m_LUTB, 0, sizeof(byte)*256);
	}
}

/** \brief 设置拉伸表 */
void QgsRasterLayer::setLookupTable(byte *pLutR, byte *pLutG, byte *pLutB)
{
	m_bISRGB = true;
	memset(m_LUTA, 0, sizeof(byte)*256);
	memcpy(m_LUTR, pLutR, sizeof(byte)*256);
	memcpy(m_LUTG, pLutG, sizeof(byte)*256);
	memcpy(m_LUTB, pLutB, sizeof(byte)*256);
}

void QgsRasterLayer::setHueSaturation(int *piHue, int *piSaturation, int *piLightness)
{
	memcpy(m_iHue, piHue, sizeof(int)*7);
	memcpy(m_iSaturation, piSaturation, sizeof(int)*7);
	memcpy(m_iLightness, piLightness, sizeof(int)*7);
}

static void CalclateHueSaturationParams(int *piHue, int *piSaturation, int *piLightness, 
										int ppiHue[6][256], int ppiSaturation[6][256], int ppiLightness[6][256])
{
#define CLAMP(x,l,u) ((x)<(l)?(l):((x)>(u)?(u):(x)))

	for (int hue=0; hue<6; hue++)
	{
		int iHueValue = static_cast<int>((piHue[0] + piHue[hue+1]) * 17.0 / 24); //色相从360归到255
		int iSaturationValue = static_cast<int>((piSaturation[0] + piSaturation[hue+1]) * 2.55); //饱和度从100归到255
		int iLightValue = static_cast<int>((piLightness[0] + piLightness[hue+1]) * 1.27); //亮度从100归到127

		for (int i=0; i<256; i++)
		{
			int iValue = iHueValue;
			/*  色相  */
			int iValuePlus = iValue + i;

			if (iValuePlus < 0)
				ppiHue[hue][i] = 255 + iValuePlus;
			else if (iValuePlus > 255)
				ppiHue[hue][i] = iValuePlus - 255;
			else
				ppiHue[hue][i] = iValuePlus;

			/*  亮度  */
			iValue = iLightValue;
			iValue = CLAMP (iValue, -255, 255); //使iValue值不超过-255到+255

			if (iValue < 0)
				ppiLightness[hue][i] = int((i * (255 + iValue)) / 255);
			else
				ppiLightness[hue][i] = int(i + ((255 - i) * iValue) / 255);

			/*  包核对  */
			iValue = iSaturationValue;
			iValue = CLAMP (iValue, -255, 255);

			ppiSaturation[hue][i] = CLAMP ((i * (255 + iValue)) / 255, 0, 255);
		}
	}
}

inline void RGB2HSL(byte &bR, byte &bG, byte &bB)
{
	int min, max;
	if (bR > bG)
	{
	...
	else
	{
		...
		if (bR == max)
			h = (bG - bB) / (double) delta;
		else if (bG == max)
			h = 2 + (bB - bR) / (double) delta;
		else
			h = 4 + (bR - bG) / (double) delta;
...
inline int hsl_value_int (double n1,double n2,double hue)
{
	double value;
...
inline void HSL2RGB(byte &bH, byte &bS, byte &bL)
{
	double h, s, l;
	...
	else
	{
		...
		/*  chromatic case  */
		bH = hsl_value_int (m1, m2, h + 85);
		bS = hsl_value_int (m1, m2, h);
		bL = hsl_value_int (m1, m2, h - 85);
	}
}

void ConvertHueSaturationRGB(int ppiHue[6][256], int ppiSaturation[6][256], int ppiLightness[6][256],
							 int &bR, int &bG, int &bB)
{
	const int hue_thresholds[7] = { 21, 64, 106, 149, 192, 234, 255 };
	...
	{
		if (r < hue_thresholds[hue_counter])
		{
			hue = hue_counter;
			break;
		}
	...
	r = ppiHue[hue][r];
	g = ppiSaturation[hue][g];
	b = ppiLightness[hue][b];
...
void QgsRasterLayer::drawMultiBandHSL( QPainter * theQPainter, QgsRasterViewPort * theRasterViewPort, const QgsMapToPixel* theQgsMapToPixel)
{
	if (bandCount() < 3)
		return;
	...
	*/
	...
	/* 计算变换关系 */
	CalclateHueSaturationParams(m_iHue, m_iSaturation, m_iLightness, hue_transfer, lightness_transfer, saturation_transfer);
		...
		&& ( !transparencyImageBuffer || transparencyImageBuffer->nextScanLine( &transparencyImageScanLine, &transparencyRasterScanLine ) ) )
	{
		for ( int i = 0; i < theRasterViewPort->drawableAreaXDim; ++i )
		{
			...
			ConvertHueSaturationRGB(hue_transfer, lightness_transfer, saturation_transfer, myStretchedRedValue, myStretchedGreenValue, myStretchedBlueValue);
