...
bool QgsVectorLayer::draw( QgsRenderContext& rendererContext )
{
  ...
  //建立一个临时的绘制对象，将数据绘制在该对象上
  QPainter PainterTemp( cachedImage );
  rendererContext.setPainter(&PainterTemp);
  ...
  if ( mUsingRendererV2 )
  {
	...
	//复制缓存，并重置绘制器
	theQPainter->drawImage( 0,0, *cachedImage );
	rendererContext.setPainter(theQPainter);
