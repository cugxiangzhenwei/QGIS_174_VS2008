...
void QgsMapCanvas::setLayerSet( QList<QgsMapCanvasLayer> &layers )
{
  ...
  for ( i = 0; i < layers.size(); i++ )
  {
    ...
    if ( lyr.isVisible() )
    {
	  ...
	  if (mSwipeLayerID.length() == 0)
		  mSwipeLayerID = lyr.layer()->getLayerID();	  
    }
...
void QgsMapCanvas::swipeAction( QMouseEvent * e )
{
	if ( mDrawing )
		return;
	...
	if (mSwipeLayerID.length() > 0)
	{		
		QgsMapLayer *ml = QgsMapLayerRegistry::instance()->mapLayer( mSwipeLayerID );
		ml->setIsSwipe(true);
		...
		if (mCurSwipeMode == SwipeLeft)		
			swipeRect = QRect(curPoint.x(),0,(iWidth - curPoint.x()),iHeight);
		else if (mCurSwipeMode == SwipeRight)		
			swipeRect = QRect(0,0,curPoint.x(),iHeight);
		else if (mCurSwipeMode == SwipeTop)		
			swipeRect = QRect(0,curPoint.y(),iWidth,(iHeight - curPoint.y()));
		else if (mCurSwipeMode == SwipeBottom)		
			swipeRect = QRect(0,0,iWidth,curPoint.y());			
		else if (mCurSwipeMode == SwipeLeft)		
			swipeRect = QRect(0,0,iWidth,iHeight);

		ml->setSwipeShowArea(swipeRect);		
	}
}

void QgsMapCanvas::swipeActionEnd(  )
{
	if ( mDrawing )
		return;

	mCurSwipeMode = SwipeNone;
...
void QgsMapCanvas::setSwipeMode(SwipeMode smode)
{
	mCurSwipeMode = smode;
}
