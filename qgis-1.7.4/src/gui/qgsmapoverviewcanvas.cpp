...
void QgsMapOverviewCanvas::refresh()
{
  ...
  //////»æÖÆGCP/////////
  const QgsMapToPixel* cXf = mMapRenderer->coordinateTransform();
  painter.setPen( mPointColor );

  for(int i = 0; i < mGcpPoints.size(); i++)
  {
	  QgsPoint qgspoint = mGcpPoints.at(i);
	  // transform the points before drawing
	  if ( cXf )
		  cXf->transform( &qgspoint );

	  QPoint point(qgspoint.x(),qgspoint.y());
...
void QgsMapOverviewCanvas::setGCPpoints(QList<QgsPoint> gcppoints, const QColor& color)
{
	mGcpPoints = gcppoints;
	mPointColor = color;
}
...
void QgsMapOverviewCanvas::ClearRect()
{
	mPanningWidget->hide();
}
