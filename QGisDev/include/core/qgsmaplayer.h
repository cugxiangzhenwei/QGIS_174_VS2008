...
class CORE_EXPORT QgsMapLayer : public QObject
{
  ...
  public:
    ...
    */
	...
	/** 设置对比度 */
	void setContrast( unsigned int );

	/** 获取Gamma. */
	unsigned int getGamma();

	/** 设置Gamma */
	void setGamma( unsigned int );

	//设置卷联状态
	void setIsSwipe(bool isSwipe);

	//设置卷帘显示区域
	void setSwipeShowArea(QRect rectshow);

	void setReDraw(bool bRedraw) { mRedraw = bRedraw; }
  ...
  protected:
    ...
    */
	...
	//add by liminlu0314@163.com
	/** 图层亮度 should be -100-100 (100最亮) */
	int mBrightnessLevel;
	
	/** 图层gamma should be 0.1-10 (默认1) */
	float mGammaLevel;
	
	/** 图层对比度 should be 0-200 (默认100) */
	unsigned int mContrastLevel;

	//标记是否需要强制刷新，在更改图层属性之后用到,比如透明度、nodatavalue等
	bool mRedraw;
	...
	//卷帘时的缓存图层，用于临时绘制透明等效果到image
	QImage *cachedImage2Swip;
