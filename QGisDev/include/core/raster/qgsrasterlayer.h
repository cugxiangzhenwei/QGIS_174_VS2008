...
class CORE_EXPORT QgsRasterLayer : public QgsMapLayer
{
  ...
  public:
    ...
    enum DrawingStyle
    {
	  ...
	  MultiBandCustomColor,            //用户自定义的颜色显示方案，用于真彩色转换
	  MultiBandLUT,					   //使用多波段LUT查找表拉伸显示
	  MultiBandHSL,					   //使用HSL变换	
    ...
    *   If no matching band is found zero will be returned! */
	...
	void setCustomBands(double const *cusbands, int iSize = 12);

	//得到当前图层自定义的颜色方案
	void getCustomBands(double *cusbands, int &iSize);

	//得到用户调色后对照表,为int型256数组
	void getLookupTable(int theBand, int *lutable);

	/** \brief 获取指定波段的统计信息及其直方图 */
	QgsRasterBandStats GetBandStats( int theBandNoInt,
		int theBinCountInt = 256,
		bool theIgnoreOutOfRangeFlag = true,
		bool theThoroughBandScanFlag = false );

	/** \brief 设置拉伸表，false为灰度图像，true为所有波段都使用该拉伸表 */
	void setLookupTable(byte *pLutG, bool bIsRGB = false);
	
	/** \brief 设置拉伸表 */
	void setLookupTable(byte *pLutR, byte *pLutG, byte *pLutB);

	/** \brief 设置色相饱和度参数 */
	void setHueSaturation(int *piHue, int *piSaturation, int *piLightness);
  ...
  private:
    ...
    // Private methods
    //
	/** \获取累计直方图后的各波段的取值范围，用来2%拉伸 */
	  void GetHistoMinMax(QgsRasterBandStats * bandStats, double &dMin,double &dMax );
	...
	/** \brief 真彩色转换函数绘制函数  */
	void drawMultiBandCustomColor( QPainter * theQPainter, QgsRasterViewPort * theRasterViewPort, const QgsMapToPixel* theQgsMapToPixel );
	
	/** \brief 真彩色转换函数绘制函数  */
	void drawMultiBandLut( QPainter * theQPainter, QgsRasterViewPort * theRasterViewPort, const QgsMapToPixel* theQgsMapToPixel );
	
	/** \brief 色相饱和度绘制函数  */
	void drawMultiBandHSL( QPainter * theQPainter, QgsRasterViewPort * theRasterViewPort, const QgsMapToPixel* theQgsMapToPixel );
	...
	//调整亮度对比度透明度
	QImage enhancedImage( const QImage& image);
